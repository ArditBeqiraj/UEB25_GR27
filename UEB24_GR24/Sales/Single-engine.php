<?php
// Përcakto variablat për header.php
$customCSS = "../css/sales.css"; // Rruga relative nga Sales/ te css/
...
// Lexo të dhënat nga aircraft_data.txt
$aircraft_data = file_get_contents('aircraft_data.txt'); // Rruga nga Sales/
$aircrafts = json_decode($aircraft_data, true);

// Gjej avionin me id="cessna180" (për Single-engine.php)
$aircraft = null;
foreach ($aircrafts as $item) {
    if ($item['id'] === 'cessna180') {
        $aircraft = $item;
        break;
    }
}
...
<head>
    ...
    <style>
        /* Stilet për header-in dhe footer-in për të ruajtur pamjen e faqes aktuale */
        nav {
